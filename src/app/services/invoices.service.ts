...
export class InvoicesService {
  ...
  getInvoices(): Observable<any> {
    const endpoint = `http://invoices.5d-agency.com/service1.svc/ViewAllInvoice?`;
    return this.http.get<any>(endpoint);
  }
  // View Invoice
  getInvoiceDetails(invoiceId): Observable<any> {
    const endpoint = `http://invoices.5d-agency.com/service1.svc/ViewInvoiceById?invoiceId=${ invoiceId }`;
    return this.http.get<any>(endpoint);
  }
  // Create Invoice
  proceedInvoice(detailsObj: any, totalsObj: any): Observable<any>{
    const endpoint = `http://invoices.5d-agency.com/service1.svc/CreateInvoice?ClientId=${ detailsObj.ClientId}&ProjectName=${detailsObj.ProjectName }&Description=${ detailsObj.Description}&PaymentMethodId=${detailsObj.PaymentMethodId }&CurrencyId=${ detailsObj.CurrencyId}&StatusId=${detailsObj.StatusId }&Taxed=${detailsObj.Taxed }&TaxId=${detailsObj.TaxId}&TotalAmount=${totalsObj.TotalAmount }&PaidAmount=${totalsObj.PaidAmount }&DueAmount=${ totalsObj.DueAmount}&SubTotal=${ totalsObj.SubTotal}&DueDate=${ detailsObj.DueDate }`;
    return this.http.get<any>(endpoint);
  }
  // Create Invoice Items
  createInvoiceItem(item: any, invoiceId: number): Observable<any>{
    const endpoint = `http://invoices.5d-agency.com/service1.svc/CreateItem?InvoiceId=${ invoiceId }&Name=${ item.itemName }&Description=${ item.itemDescription }&UnitPrice=${ item.unitPrice }&Quantity=${ item.quantity }&TotalPrice=${ item.quantity * item.unitPrice }`;
    return this.http.get<any>(endpoint);
  }
  // Edit Invoice
  editInvoice(detailsObj: any, totalsObj: any): Observable<any>{
    const endpoint = `http://invoices.5d-agency.com/service1.svc/EditInvoice?InvoiceId=${ detailsObj.InvoiceId }&DueDate=${ detailsObj.DueDate }&ClientId=${ detailsObj.ClientId }&ProjectName=${ detailsObj.ProjectName }&Description=${ detailsObj.Description }&PaymentMethodId=${ detailsObj.PaymentMethodId }&CurrencyId=${ detailsObj.CurrencyId }&StatusId=${ detailsObj.StatusId }&Taxed=${ detailsObj.Taxed }&TaxId=${ detailsObj.TaxId }&PaidAmount=${ totalsObj.PaidAmount }&DueAmount=${ totalsObj.DueAmount }&SubTotal=${ totalsObj.SubTotal }&TotalAmount=${totalsObj.TotalAmount }`;
    return this.http.get<any>(endpoint);
  }
  // Edit Invoice Items
  editInvoiceItem(item: any): Observable<any>{
    const endpoint = `http://invoices.5d-agency.com/service1.svc/EditItems?ItemId=${ item.id }&Description=${ item.itemDescription }&UnitPrice=${ item.unitPrice }&Quantity=${ item.quantity }&Name=${ item.itemName }&TotalPrice=${ item.quantity * item.unitPrice }`;
    return this.http.get<any>(endpoint);
  }
  // Delete Invoice Item
  deleteInvoiceItem(itemId: number): Observable<any>{
    const endpoint = `http://invoices.5d-agency.com/service1.svc/DeleteItem?ItemId=${ itemId }`;
    return this.http.get<any>(endpoint);
  }
  // Delete Invoice
  deleteInvoice(invoiceId: number): Observable<any>{
    const endpoint = `http://invoices.5d-agency.com/service1.svc/DeleteInvoice?InvoiceId=${ invoiceId }`;
    return this.http.get<any>(endpoint);
  }
